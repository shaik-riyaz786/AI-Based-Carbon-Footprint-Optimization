...
from wtforms.validators import DataRequired, Email, EqualTo, Length, Regexp, ValidationError  # Added ValidationError
...
from tkinter import _tkinter  # Optional, for deeper suppression
import matplotlib
...
# Function to generate the report (unchanged)
def generate_report(file_path):
    try:
        preprocessor = joblib.load(r'preprocessing_weights\Preprocessor.pkl')
        xgb_model = joblib.load(r'Model_weights\xgboost_model.pkl')
        ...
        if df['CO₂e_Total'].isna().all():
            ...
            output_path = file_path.replace('.', '_predicted.')
            if file_path.endswith('.csv'):
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if existing_user and existing_email:
            flash('Both username and email are already in Database, please try different uername and Email', 'error')
...
def dashboard():
    ...
    if form.validate_on_submit():
        ...
        if file and file.filename.endswith(('.csv', '.xlsx', '.xls')):
            ...
            file.save(file_path)
            esg_summary, image_base64, predicted_data, error = generate_report(file_path)
            if error:
                flash(error, 'error')
            else:
